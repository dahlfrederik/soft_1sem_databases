...
CREATE OR REPLACE FUNCTION generate_people(amount_to_generate int, source_table_name TEXT)
	RETURNS TABLE (cpr TEXT, firstname TEXT, lastname TEXT, gender TEXT, birthday timestamp, phone TEXT, email TEXT)
	LANGUAGE plpgsql
	VOLATILE
AS
		...
		SELECT 
			...
			FROM
				(SELECT tabl.cpr, tabl.firstname, tabl.lastname, tabl.gender, tabl.birthday, tabl.phone, tabl.email 
					FROM generate_person(source_table_name) AS tabl --we name the table from generate_person() tabl
				) 
